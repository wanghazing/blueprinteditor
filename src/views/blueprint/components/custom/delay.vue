<template>
  <div class="node-component node-delay" @contextmenu="onNodeContextMenu">
    <h1
    ...
    <div class="node-component--rows">
      ...
      <div class="row-item row-item__add" v-if="addRuleFlag">
        <input type="text" @blur="onAddRuleFinish" placeholder="仅支持整数" />
      </div>
...
const props = defineProps(["title", "rows", "isActive", "nodeId", "editFlag"]);
...
const onAddRuleFinish = (e: FocusEvent) => {
  ...
  if (value && !isNaN(Number(value))) {
    blueprintStore.addNodeRule(props.nodeId, Number(value).toFixed(0));
  }
...
.node-delay {
  .node-component--title {
    background-color: rgba(226, 13, 70, 0.81) !important;
  }
}
</style>
