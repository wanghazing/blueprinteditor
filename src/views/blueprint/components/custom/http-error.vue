<template>
  <div class="node-component node-http-error" @contextmenu="onNodeContextMenu">
    <h1
    ...
    <div class="node-component--rows">
      ...
      <div class="row-item row-item__add" v-if="addRuleFlag">
        <input type="text" @blur="onAddRuleFinish" placeholder="http错误码" />
      </div>
...
const props = defineProps(["title", "rows", "nodeId", "isActive", "editFlag"]);
...
const onAddRuleFinish = (e: FocusEvent) => {
  ...
  if (value && value.length === 3 && !isNaN(code) && code % 1 === 0) {
...
.node-http-error {
  .node-component--title {
    background-color: rgba(202, 62, 6, 0.81) !important;
  }
}
</style>
